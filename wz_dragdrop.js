...
{
  //if(dd.obj.name == "char0") return;
  if(wDivXY[0] == "") {
    for(cc = 0; cc < curWord.length; cc++) {
      var myDiv = eval('dd.elements.char' + cc);
      wDivXY[cc] = '' + myDiv.x + ',' + myDiv.y + '';
    }
  }
  ...
  for(cc = 0; cc < curWord.length; cc++) {
    var myDiv = eval('dd.elements.anchor' + cc);
    aDivXY[cc] = '' + myDiv.x + ',' + myDiv.y + '';
    //alert("CharXY: " + wDivXY[cc] + "\nAnchorXY: " + aDivXY[cc]);
  }
}
...
function my_DropFunc()
{
  ...
  for(cc = 0; cc < curWord.length; cc++) {
    var myDiv = eval('dd.elements.anchor' + cc);
    var lessX = myDiv.x - 10;
    var moreX = myDiv.x + 15;
    var lessY = myDiv.y - 10;
    var moreY = myDiv.y + 15;

    if(curX > lessX && curX < moreX && curY > lessY && curY < moreY) {
      for(cw = 0; cw < curWord.length; cw++) {
        var cwDiv = eval('dd.elements.char' + cw);
        if(cwDiv.x == myDiv.x && cwDiv.y == myDiv.y) { //this char div is anchored already to the current anchor
          cwDiv.moveTo(cwDiv.defx, cwDiv.defy);
          break;
        }
      }

      dd.obj.moveTo(myDiv.x, myDiv.y);
      checkAnchored();
      return;
    }
  }
...
function checkAnchored() {
  var anchoredDivNum = 0;
  for(cc = 0; cc < curWord.length; cc++) {
    var thisWDiv = eval('dd.elements.char' + cc);

    for(ac = 0; ac < curWord.length; ac++) {
      var thisADiv = eval('dd.elements.anchor' + ac);
      if(thisADiv.x == thisWDiv.x && thisADiv.y == thisWDiv.y) { //the layer is matched to an anchor
        anchoredDivNum++;
      }
    }
  }

  if(anchoredDivNum == curWord.length) { //all divs are anchored, so check answer
    checkAnswer();
  }
}
...
function checkAnswer() {
  var tempStr = "";
  for(cw = 0; cw < curWord.length; cw++) {
    var myADiv = eval('dd.elements.anchor' + cw);
    for(ac = 0; ac < curWord.length; ac++) {
      var myWDiv = eval('dd.elements.char' + ac);
      if(myADiv.x == myWDiv.x && myADiv.y == myWDiv.y) { //the layer is matched to this anchor
        if(document.getElementById("char" + ac).innerHTML == "&nbsp;") tempStr += " ";
        else tempStr += document.getElementById("char" + ac).innerHTML;
      }
    }
  }
  ...
  if(tempStr == curWord.toUpperCase()) { //all the letters are correct
    clearInterval(timerID);
    score++;
    document.getElementById("correct").style.top = document.body.clientHeight/2 - 150;
    document.getElementById("correct").style.left = document.body.clientWidth/2 - 200;
    document.getElementById("correct").style.display = '';
    document.getElementById("correctContinue").focus();
  } else {
    wordArray[curWordIndex]["missed"] = true;
    document.getElementById("wrong").style.top = document.body.clientHeight/2 - 150;
    document.getElementById("wrong").style.left = document.body.clientWidth/2 - 200;
    document.getElementById("wrong").style.display = '';

    if(keepTrying) {
      document.getElementById("wrongContinue").value = "TRY AGAIN";
    ...
    document.getElementById("wrongContinue").focus();
  }
  var percent = Math.round((score/(curIndex + 1)) * 100);
  document.getElementById("curScore").innerHTML = "Score: " + score + "(" + percent + "%)";
}